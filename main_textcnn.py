...
if __name__ == '__main__':
    ...
    # model
    textcnn_model = TextCNN(n_classes=n_classes,
                            embed_size=EMBED_SIZE,
                            vocab_size=len(func_tok2idx),
                            kernel_windows=[3, 4, 5],
                            input_channel=1,
                            pre_embedding=embed_model)
    logger.info(textcnn_model)
    textcnn_model = textcnn_model.to(DEVICE)
    optimizer = torch.optim.Adam(textcnn_model.parameters(), lr=learning_rate)
    ...
    challe_trainer = ChallengeTrainer(model=textcnn_model,
                                      optimizer=optimizer,
                                      criterion=criterion,
                                      device=DEVICE,
    ...
    writer = SummaryWriter(f'logs/{textcnn_model.name}/{now}')
